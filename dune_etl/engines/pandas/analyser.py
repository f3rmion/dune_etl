...
class PandasAnalyser(Analyser):
    ...
    def create_analysis(self, engine="pyarrow"):
        ...
        self.vertical_df = self.vertical_df[
            ~self.vertical_df["vertical"].str.contains(
                "unclassified", case=False, na=False
            )
            & ~self.vertical_df["vertical"].str.contains(
                "burn address", case=False, na=False
            )
        ]
        ...
        self.protocol_df = self.protocol_df[
            ~self.protocol_df["protocol"].str.contains(
                "unclassified", case=False, na=False
            )
            & ~self.protocol_df["protocol"].str.contains(
                "burn address", case=False, na=False
        ...
        self.vertical_df[numeric_columns] = self.vertical_df[numeric_columns].apply(
            pd.to_numeric
        )
        self.protocol_df[numeric_columns] = self.protocol_df[numeric_columns].apply(
            pd.to_numeric
        )
        ...
        top5_vertical_by_transactions = grouped_verticals.nlargest(
            5, "total_transactions"
        )

        top5_protocol_by_tvp = grouped_protocols.nlargest(5, "total_tvp")
        top5_protocol_by_transactions = grouped_protocols.nlargest(
            5, "total_transactions"
        )
